...
// SavedInsight represents a user-saved insight query
...
func (s *SavedInsight) BeforeCreate(tx *gorm.DB) (err error) {
	if s.ID == "" {
		id, err := gonanoid.New(21)
		if err != nil {
			return err
...
// SavedRetention represents a user-saved retention query
...
func (s *SavedRetention) BeforeCreate(tx *gorm.DB) (err error) {
	if s.ID == "" {
		id, err := gonanoid.New(21)
		if err != nil {
			return err
...
// SavedFlow represents a user-saved Flow query
...
func (s *SavedFlow) BeforeCreate(tx *gorm.DB) (err error) {
	if s.ID == "" {
		id, err := gonanoid.New(21)
		if err != nil {
			return err
