...
type LexiconEvent struct {
	...
	Name        string         `gorm:"not null;index" json:"name"` // The raw event name (e.g., "signup_completed")
...
type LexiconEventProperty struct {
	ID           uint           `gorm:"primaryKey" json:"id"`
	ProjectID    uint           `gorm:"not null;index" json:"project_id"`
	EventID      *uint          `gorm:"index" json:"event_id"`      // Null for global properties, set for event-specific
	Name         string         `gorm:"not null;index" json:"name"` // The raw property name (e.g., "browser")
